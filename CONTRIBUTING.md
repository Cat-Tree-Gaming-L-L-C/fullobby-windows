# Contributing to CHLL Seeding

Thanks for your interest in improving the CHLL Seeding client! This is a
...
`tools/ChllSeeding.MockApi/` (`scripts/run-mock.ps1`).

## Making changes

- **Keep business logic in `ChllSeeding.Core`** (no XAML dependencies) so it
  stays unit-testable; the `ChllSeeding.App` layer owns UI and lifecycle.
...
Open a [GitHub issue](https://github.com/Cat-Tree-Gaming-L-L-C/chll-seeding-windows/issues)
