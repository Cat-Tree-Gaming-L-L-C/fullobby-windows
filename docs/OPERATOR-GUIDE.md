...
- Register the hub channel, move the org's Discord → your Org Admin
- *Changing* the network's join code, its seeding hours, pausing it, rotation
  order across orgs → a Network Admin (a grant over the network). You can **read** the join code yourself with
