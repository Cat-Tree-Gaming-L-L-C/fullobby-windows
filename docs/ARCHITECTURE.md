...
| Seeding networks | Multi-tenant orgs (limited-beta gate): membership via `SeedingApiClient` (`PreviewInviteAsync`/`AcceptInviteAsync` for invite links — token read by `InviteLink.ParseToken`, also reachable via `fullobby://invite?token=` — and the legacy `JoinNetworkAsync`/`GetMyNetworksAsync`/`LeaveNetworkAsync`/`SetNetworkPrioritiesAsync`), state + hard gate in `AccountViewModel` (onboarding network step; re-armed portal on session restore with zero memberships or a `join_a_network` directive — fails open on fetch errors). Seeding status is per network (`NetworkSeedingStatus`); the seed board flattens/groups by network. Invite tokens and join codes are sent and forgotten — never stored client-side. |
...
| Game catalog | `Core.Games.GameDefinition` + `GameCatalog` (plain data record — no per-game interface); `InstalledGames` (Steam manifests → the launchable subset sent as the directive's `games=`). Process identity is exe name **plus** the `steamapps\common\<InstallFolder>\` path segment (`ProcessMonitor.IsGameSteamPath`): both HLL titles ship `Launch_HLL.exe`, and "Hell Let Loose" is a prefix of "Hell Let Loose - Vietnam" |
...
| Startup | `Core.Platform.StartupRegistry` (HKCU Run `Fullobby`, data `"<exe>" --minimized`) — the flag (`Branding.MinimizedArg`) makes `App.OnLaunched` skip `_window.Activate()`, so a sign-in launch comes up in the tray with no window; not activating *is* the mechanism (a WinUI window is created hidden), so there's no show-then-hide flash, and what it defers is `RootGrid.Loaded` plus the unprotected-secrets dialog that already guards on a null `XamlRoot`. `UpdatePathIfNeeded` compares the whole command, so entries written before the flag existed are rewritten on the next launch. One entry, two writers: the Settings toggle and the installer's `startup` task (`installer/Fullobby.iss`), an **opt-out** pre-ticked on a first install only. `InitializeWizard` re-seeds the tick from the live Run value on a reinstall/update (the updater runs setup.exe interactively, so the tasks page appears on every update and must not resurrect a startup the user turned off); clearing the box deletes the value in `CurStepChanged`. The value data must stay byte-identical to `StartupRegistry`'s quoted exe path, which `UpdatePathIfNeeded` compares against |
