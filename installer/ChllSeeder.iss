; CHLL Seeder — Inno Setup installer (unpackaged WinUI 3, no MSIX)
;
; Build:
;   dotnet publish src/ChllSeeder.App -c Release -r win-x64 --self-contained true -p:Platform=x64 -o artifacts/publish
;   iscc installer\ChllSeeder.iss [/DAppVersion=x.y.z] [/DPublishDir=path]
;
; Output: installer\Output\CHLL-Seeder-Setup-<ver>.exe
...
#define AppName "CHLL Seeder"
#define AppPublisher "Comp HLL"
#define ExeName "CHLLSeeder.exe"
#define Scheme "chllseeder"
#define RepoUrl "https://github.com/catalloc/chll-seeder-windows"
...
OutputDir=Output
OutputBaseFilename=CHLL-Seeder-Setup-{#AppVersion}
...
; Gracefully close a running CHLL Seeder (Restart Manager) before updating
...
[Registry]
; chllseeder:// deep-link protocol (OAuth callbacks).
; HKA resolves to HKCU under PrivilegesRequired=lowest.
Root: HKA; Subkey: "Software\Classes\{#Scheme}"; ValueType: string; ValueName: ""; ValueData: "URL:CHLL Seeder Protocol"; Flags: uninsdeletekey
