...
public sealed class MockState
{
    ...
    private readonly List<NetworkMembership> _memberships = new();
    private int _seq;
    private long _membershipSeq;

    // The single mock seeding network. Join with name "mock" + code "TEST-2345-CODE".
    public const string NetworkName = "mock";
    public const string NetworkDisplayName = "Mock Network";
    public const string JoinCode = "TEST-2345-CODE";
    ...
    public void ResetToDefaults()
    {
        lock (_gate)
        {
            ...
            _memberships.Clear();
            _memberships.Add(new NetworkMembership(++_membershipSeq, NetworkId,
                NetworkName, NetworkDisplayName, NetworkInviteUrl, 0, DateTimeOffset.UtcNow.ToUnixTimeSeconds()));
        }
        BroadcastState();
    }
    ...
    /// <summary>Per-network seeding status: one mock network carrying the whole rotation. The
    /// candidate is the first online, non-passworded server still under its threshold (lowest
    /// index wins — sequential rotation); null when none qualify.</summary>
    ...
    /// <summary>Per-server standing for the client's board. The mock doesn't model seed windows, so
    /// it emits current/done/skipped/pending only (not_ready/missed_ready/deferred are window states).</summary>
    private List<ServerDayStatus> DayStatuses()
    {
        var candidateIndex = Candidate()?.Index;
        return _servers.OrderBy(s => s.Index).Select(s =>
        {
            var status =
                s.Index == candidateIndex ? "current"
                : s.PlayerCount is { } pc && pc >= s.Info.SeedingThreshold ? "done"
                : s.Offline || s.PasswordProtected ? "skipped"
                : "pending";
            return new ServerDayStatus(
                s.Info.BmId, s.Info.Name, s.Info.ShortName, NetworkName, status,
                s.PlayerCount, s.Info.SeedingThreshold, null);
        }).ToList();
    }
    ...
    public SeedingDirective Directive(int? currentIndex)
    {
        lock (_gate)
        {
            ...
            {
                return new SeedingDirective("stop", null, AllExhausted: false, ScheduledPause: false,
                    ...
                    PollAgainInSecs: config.PollIdleSecs, MaxSessionSecs: config.MaxSessionSecs,
                    Config: config, JoinANetwork: true);
            }
            ...
            // Classify why we're switching the client off its current server.
            string? switchReason = null;
            if (action == "switch" && currentIndex is { } cur)
            {
                var currentServer = _servers.FirstOrDefault(s => s.Index == cur);
                switchReason =
                    currentServer is { } cs && cs.PlayerCount is { } pc && pc >= cs.Info.SeedingThreshold ? "seeded"
                    : currentServer is { Offline: true } or { PasswordProtected: true } ? "unavailable"
                    : target.Index < cur ? "higher_priority_ready"
                    : "rotation_advanced";
            }
    ...
    public List<NetworkMembership> Memberships
    {
        get { lock (_gate) { return _memberships.OrderBy(m => m.Priority).ToList(); } }
    }

    /// <summary>Join the mock network. Null on a bad tag/code (the caller returns the uniform 400);
    /// idempotent — joining again returns the existing membership.</summary>
    public NetworkMembership? Join(string name, string code)
    {
        if (!string.Equals(name, NetworkName, StringComparison.OrdinalIgnoreCase) || code != JoinCode)
        {
            return null;
        }
        lock (_gate)
        {
            var existing = _memberships.FirstOrDefault(m => m.NetworkId == NetworkId);
            if (existing is not null)
            {
                return existing;
            }
            var m = new NetworkMembership(++_membershipSeq, NetworkId, NetworkName,
                NetworkDisplayName, NetworkInviteUrl, _memberships.Count, DateTimeOffset.UtcNow.ToUnixTimeSeconds());
            _memberships.Add(m);
            return m;
        }
    }
    ...
    public void Leave(long networkId)
    {
        lock (_gate) { _memberships.RemoveAll(m => m.NetworkId == networkId); }
    }

    /// <summary>Reorder priorities. Null when the ids don't exactly match the membership set.</summary>
    public List<NetworkMembership>? Reorder(List<long> orderedIds)
    {
        lock (_gate)
        {
            if (orderedIds.Count != _memberships.Count
                || _memberships.Any(m => !orderedIds.Contains(m.NetworkId)))
            {
                return null;
            }
            var reordered = orderedIds
                .Select((id, i) => _memberships.First(m => m.NetworkId == id) with { Priority = i + 1 })
                .ToList();
            _memberships.Clear();
            _memberships.AddRange(reordered);
            return reordered;
        }
    }
