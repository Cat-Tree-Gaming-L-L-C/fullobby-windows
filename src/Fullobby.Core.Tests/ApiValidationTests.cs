...
public class ApiValidationTests
{
    ...
    [Theory]
    [InlineData("""API error: {"error":"Rate limited"}""")]
    [InlineData("API error: 429 Too Many Requests")]
    public void FriendlyError_RateLimited(string input) =>
        Assert.Equal("Too many attempts — please wait a minute and try again",
            ApiValidation.FriendlyError(input));
