...
public class ManualBackupServiceTests
{
    ...
    {
        var missing = Path.Combine(Path.GetTempPath(), "fullobby-does-not-exist-" + Guid.NewGuid().ToString("N"));
    ...
    {
        var dir = Path.Combine(Path.GetTempPath(), "fullobby-validate-" + Guid.NewGuid().ToString("N"));
