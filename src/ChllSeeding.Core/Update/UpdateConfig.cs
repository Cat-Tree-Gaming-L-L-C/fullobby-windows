...
/// (always HTTPS) and cannot be redirected — the <c>CHLL_SEEDING_UPDATE_FEED_URL</c> override is
...
///
/// <para>Debug builds honor the <c>CHLL_SEEDING_UPDATE_FEED_URL</c> override (handy for pointing the
/// updater at a locally-served, locally-signed manifest during testing).</para></summary>
public static class UpdateConfig
{
#if DEBUG
    public static string FeedBaseUrl =>
        Environment.GetEnvironmentVariable("CHLL_SEEDING_UPDATE_FEED_URL") is { Length: > 0 } overridden
            ? overridden.TrimEnd('/')
