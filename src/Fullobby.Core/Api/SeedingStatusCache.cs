...
public sealed class SeedingStatusCache(SeedingConfigProvider configProvider)
{
    ...
    /// <summary>Raised (on the caller's thread) whenever a fresh status is stored, so the UI can
    /// refresh the per-server board. Handlers must marshal to the UI thread themselves.</summary>
    public event Action<SeedingStatusResponse>? Updated;
