...
public static class ApiValidation
{
    ...
    /// <summary>URL for the Turnstile challenge page shown before guest registration when the
    /// server requires a verified token (see <c>DeepLinkAction.RegisterCallback</c>).</summary>
    public static string GetRegisterChallengeUrl(string state) =>
        $"{ApiConfig.BaseUrl}/register-challenge?state={Uri.EscapeDataString(state)}";
