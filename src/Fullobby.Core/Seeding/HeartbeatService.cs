...
/// </summary>
public sealed class HeartbeatService(SeedingApiClient api, SeedingConfigProvider configProvider, SeedingEngine engine, ILogger<HeartbeatService> log)
{
    ...
    private async Task RunAsync(string sessionId, CancellationToken ct)
    {
        ...
        while (!ct.IsCancellationRequested)
        {
            ...
            try
            {
                ...
                // The server closed the session in-band (e.g. an unrecoverable drop hit the
                // validation limit). Tell the engine to stop, then end this loop.
                if (IsTerminalStatus(resp.Status))
                {
                    log.LogWarning("Server reported session status '{Status}'; ending heartbeat", resp.Status);
                    engine.OnSessionEnded(resp.Status);
                    break;
                }

                // Act on a server-issued restart-and-rejoin signal (or clear stale rejoin state).
                engine.OnHeartbeatResponse(resp);
            }
            catch (OperationCanceledException) when (ct.IsCancellationRequested)
            {
                break;
            }
            catch (ApiException ae) when (ae.StatusCode is HttpStatusCode.BadRequest or HttpStatusCode.NotFound)
            {
                // The session no longer exists server-side — e.g. the join-stall give-up closed it,
                // or it expired between sweeps. This is definitive (not a transient error), so stop
                // trying and let the engine close the game and surface a message.
                log.LogWarning(ae, "Seeding session is no longer active; ending heartbeat");
                engine.OnSessionEnded("session_closed");
                break;
            }
            catch (Exception e)
            {
    ...
    /// <summary>Whether a heartbeat response's session status means the session is over (the server
    /// won't accept further heartbeats). "active" is the only non-terminal state.</summary>
    private static bool IsTerminalStatus(string status) =>
        status is "failed_validation" or "expired" or "completed";
